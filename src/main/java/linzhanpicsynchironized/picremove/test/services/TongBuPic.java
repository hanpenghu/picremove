package linzhanpicsynchironized.picremove.test.services;


import linzhanpicsynchironized.picremove.test.CONST.C;
import linzhanpicsynchironized.picremove.test.dto.OnePrdtOneThumbSomeAttachUrl;
import linzhanpicsynchironized.picremove.test.dto.ThumIdAndAttachId;
import linzhanpicsynchironized.picremove.test.hanhan.p;
...
//            service.scheduleAtFixedRate(myThread, this.firstInitDelyTime,this.fixdelyTime, TimeUnit.SECONDS);
    ...
    public void zuHe(){
        List<String> allPrdIds = getAllPrdId();
        if(p.notEmpty(allPrdIds)){
            for(String prdId:allPrdIds){
                if(p.notEmpty(prdId)){
                    ThumIdAndAttachId thumIdAndAttachId = getOneThumbnailIDAndSomeAttach(prdId);
                    if(p.notEmpty(thumIdAndAttachId)){
                        OnePrdtOneThumbSomeAttachUrl onePrdtOneThumbSomeAttachUrl = getOnePrdtOneThumbSomeAttachUrl(thumIdAndAttachId);
                        if(p.notEmpty(onePrdtOneThumbSomeAttachUrl)){
                            setMyUrlOfThumAndAttach(onePrdtOneThumbSomeAttachUrl);
                        }
                    }
                }
            }
